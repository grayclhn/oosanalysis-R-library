...
clarkwest <- function(null, alt, dataset, R, vcv = var,
  ...
  clarkwest_calculation(
    target        = extract_target(null, dataset),
...
passes everything to the [[dmw_calculation]] function to generate the
final statistics.  It might seem a little silly to have
[[mixedwindow]] and [[mixedwindow_calculation]] as distinct functions,
but we're going to reuse [[mixedwindow_calculation]] in the bootstrap.
...
mixedbootstrap <- function(null, alt.list, dataset, R, nboot, blocklength,
  ...
  rbootindex <-
    switch(bootstrap,
           moving = function()
             bootindex_movingblock(nobs - R, blocklength),
           circular = function()
             bootindex_circularblock(nobs - R, blocklength))
  replicate(nboot, {
...
export(clarkwest, mixedwindow, mixedbootstrap,
       recursive_forecasts, stepm, dmw_calculation,
       bootindex_circularblock, bootindex_movingblock, extract_target,
       extract_predictors)
@
