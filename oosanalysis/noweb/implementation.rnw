...
dmw_mse <- function(null, alt, dataset, R, vcv = var,
  ...
  dmw_calculation(f = errors.null^2 - errors.alt^2,
...
mccracken_criticalvalue <- 
  function(pi, k2, size, window = c("recursive", "rolling", "fixed")) {
    window <- match.arg(window)
    sizeIndex <- match(size, c(0.90, 0.95, 0.99))
    if (pi <= PiBounds[1]) {
      return(McCrackenData[[window]][1, sizeIndex, k2])
    } else if (pi >= PiBounds[2]) {
      return(McCrackenData[[window]][length(PiIntervals), sizeIndex, k2])
    } else {
      ...
      critLeft <- McCrackenData[[window]][leftIndex, sizeIndex, k2]
      critRight <- McCrackenData[[window]][leftIndex + 1, sizeIndex, k2]
...
leftcrit <- if (is.na(lefttail)) -Inf else 
              quantile(bootmin[bootmax <= rightcrit], lefttail)
...
McCrackenData <- list()
McCrackenData[["rolling"]] <- array(dim = c(12, 3, 10), dimnames = c("pi", "size", "k2"), 
  data = 
...
McCrackenData[["recursive"]] <- array(dim = c(12, 3, 10), dimnames = c("pi", "size", "k2"), 
  data =
...
McCrackenData[["fixed"]] <- array(dim = c(12, 3, 10), dimnames = c("pi", "size", "k2"), 
  data =
...
export(clarkwest, mixedwindow, mixedbootstrap, recursive_forecasts, stepm,
       ...
       predict.CT, predict.Aggregate, rvar, bootindex_stationary, mccracken_criticalvalue)
