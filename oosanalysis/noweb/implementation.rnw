...
\title{Implementation details: oosanalysis package\footnote{Copyright
    \textcopyright\ 2011 Gray Calhoun}} 
...
dmw.calculation <- function(f, h, BF, R, vcv, 
  ...
  hBF <- tcrossprod(as.matrix(h), matrix(BF, nrow = 1))
  S <- vcv(cbind(f, hBF))
