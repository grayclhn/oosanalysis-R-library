...
\title{Implementation details of the `oosanalysis'
  package\\{\large Supplemental appendix 2 for ``An
    asymptotically normal out-of-sample test of equal predictive
    ability for nested models''}\footnote{Copyright \textcopyright\ 2011 by Gray Calhoun
...
mixedbootstrap <- function(null, alt.list, dataset, R, nboot, blocklength,
  ...
  replicate(nboot, {
    ...
    sapply(forecasts.alt, function(alt) {
      estimates <- mixedwindow_calculation(y[extendedindex],
                     X[extendedindex,,drop=FALSE],
                     recursive_forecasts(null, dataset[extendedindex,], 
                                         R, "recursive"),
                     alt[bootindex], vcv)
      with(estimates, mu * sqrt(noos / avar))
  })})
}
@ %def mixedbootstrap
