...
mccracken_criticalvalue <- 
  function(pi, k2, size, window = c("recursive", "rolling", "fixed")) {
    ...
    pivalue <- findInterval_presorted(pi, PiBounds)
    McCrackenData[[window]][pivalue,sizeIndex,k2]
  }
...
<<mccrackendata.R>>=
PiBounds <- c(0, 0.05, 0.15, seq(from = 0.3, to = 1.9, by = .2))
McCrackenData <- list()
