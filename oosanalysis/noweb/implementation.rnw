...
\usepackage{noweb,setspace,amsmath,amsthm,amssymb,microtype,eco,ragged2e}
\usepackage{hyperref}
...
\title{Implementation details: `oosanalysis' package\footnote{Version
    0.2.0.  Copyright \textcopyright\ 2011 by Gray Calhoun}}
...
Package: oosanalysis
Type: Package
Title: Pseudo out-of-sample forecasting
Version: 0.2.0
Date: 2011-01-28
Author: Gray Calhoun
Maintainer: Gray Calhoun <gcalhoun@iastate.edu>
Description: Functions for out-of-sample comparisons of forecasting models
Depends: R(>= 2.10), methods, MASS
Suggests: zoo, dyn
License: GPL (>= 3)
LazyLoad: yes
LazyData: yes
...
<<clarkwest.R>>=
clarkwest.calculation <- function(target, null.forecast, 
                                  alt.forecast, vcv) {
  P <- length(target)
...
mixedwindow <- function(null, alt, dataset, R, vcv = var,
  ...
  estimates <-
    mixedwindow.calculation(y = extract.target(null, dataset),
                            ...
                            vcv = vcv)
  estimates$pvalue <-
    with(estimates, pnorm(mu, 0, sqrt(avar / (nobs - R)), FALSE))
  return(estimates)
}
@ [[mixedwindow.calculation]] constructs the \oos\ processes and then %def mixedwindow
...
@ %def mixedwindow.calculation

The [[mixedbootstrap]] function uses [[mixedwindow.calculation]] to
...
\citep{PoR:92} bootstraps.  The [[extendedindex]] vector automatically
...
mixedbootstrap <- function(null, alt.list, dataset, R, nboot, blocklength,
  ...
  rbootindex <-
    switch(bootstrap,
           moving = function()
             bootindex.movingblock(nobs - R, blocklength),
           circular = function()
             bootindex.circularblock(nobs - R, blocklength))
  replicate(nboot, {
...
@ One nice feature of this bootstrap is that it's only necessary to %def mixedbootstrap
...
@ %def bootindex.movingblock bootindex.circularblock
...
stepm <- function(teststatistics, bootmatrix, level) {
  ...
  repeat {
    ...
    if (nrejected.exit == nrejected.entry || nrejected.exit == nstatistics) break
  }
...
@ %def recursive.forecasts

\subsection{Model manipulation}

[[extract.target]] and [[extract.predictors]] are designed to take the
...
@ %def extract.predictors extract.target
...
<<NAMESPACE>>=
export(clarkwest, mixedwindow, mixedbootstrap,
       recursive.forecasts, stepm, dmw.calculation,
       bootindex.circularblock, bootindex.movingblock, extract.target,
       extract.predictors)
@
...
% LocalWords:  extendedindex NAMESPACE
