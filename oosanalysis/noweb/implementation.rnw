...
rvar <- function(nobs, coefficients, intercept, vcv, 
  ...
  y <- rbind(matrix(y0, nlag)[nlag:1,], mvrnorm(nobs, rep(0, neq), vcv))
  for (i in 1:nobs) {
...
as.data.frame(do.call("cbind", lapply(1:neq, function(j) {
    jmatrix <- sapply(nlag:1, function(i) y[(1:nobs) + i, j])
    colnames(jmatrix) <- c(eqnames[j], paste(eqnames[j], 1:(nlag-1), sep = "L"))
    jmatrix
  })))
@ 
